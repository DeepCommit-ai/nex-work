...
const KILLABLE_NAMES = new Set(['electron', 'nexwork', 'nexwork.exe', 'aionui', 'aionui.exe']);
...
function listLikelyConflictingProcesses() {
  try {
    if (isWindows()) {
      const output = run(
        "powershell -NoProfile -Command \"Get-Process | Where-Object { $_.ProcessName -in @('electron','NexWork','AionUi','node','bun') } | Select-Object ProcessName,Id | ConvertTo-Json -Compress\""
      );
      const parsed = output ? JSON.parse(output) : [];
      return Array.isArray(parsed) ? parsed : [parsed];
    }

    const output = run(`ps -A -o pid=,comm= | egrep "electron|NexWork|AionUi|node|bun" || true`);
    return output
      .split(/\r?\n/)
      .filter(Boolean)
      .map((line) => {
...
function cleanupByName() {
  ...
  for (const proc of processes) {
    ...
    if (!['electron', 'nexwork', 'aionui'].some((k) => rawName.includes(k))) continue;
    if (killPid(pid)) {
